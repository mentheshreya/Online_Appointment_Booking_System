...
export default function DoctorProfile() {
  ...
  const [formData, setFormData] = useState({ name: '', email: '', phone: '', specialization: '', experience: '', location: '', qualification: '' });
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          setUser({
            ...
            qualification: data.profile.clinicName,
    ...
    <div>
      <style jsx>{`
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .profile-container{background:var(--card-bg);border:2px solid var(--primary-green-border);border-radius:18px;padding:36px 40px;display:flex;gap:36px;margin-bottom:30px;box-shadow:0 6px 20px var(--nav-shadow);}
        
        .avatar-section{position:relative;flex-shrink:0;width:140px;height:140px;border-radius:50%;background:var(--primary-green-bg);border:3px solid var(--primary-green-border);display:flex;align-items:center;justify-content:center;font-size:4rem;overflow:hidden;cursor:pointer;transition:transform .2s;}
        .avatar-section:hover{transform:scale(1.02);border-color:var(--primary-green);}
        .avatar-section img{width:100%;height:100%;object-fit:cover;}
        
        .camera-badge{position:absolute;bottom:5px;right:5px;background:var(--primary-green);color:#ffffff;border:2px solid #fff;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:1.1rem;cursor:pointer;box-shadow:0 4px 10px var(--shadow-green);transition:all .2s;}
        .camera-badge:hover{background:var(--primary-green-border);transform:scale(1.1);}
        
        .info-section{flex:1;}
        .info-section h3{font-size:2.2rem;font-weight:900;color:var(--text-color);margin-bottom:14px;}
        .info-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;}
        .info-row{display:flex;align-items:center;gap:12px;font-size:1.05rem;color:var(--text-muted);}
        .info-row .icon{background:var(--primary-green-bg);color:var(--primary-green);width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:1rem;}
        .info-row strong{color:var(--text-color);font-weight:800;}
        
        .edit-btn{background:var(--primary-green);color:#ffffff;border:none;border-radius:999px;padding:12px 32px;font-family:'Nunito',sans-serif;font-weight:800;font-size:.95rem;cursor:pointer;transition:all .2s;opacity:0.95;margin-top:20px;display:inline-block;}
        .edit-btn:hover{opacity:1;transform:translateY(-2px);box-shadow:0 6px 15px var(--shadow-green);}
        ...
        .input-group input{border:2px solid var(--primary-green-border);border-radius:10px;padding:12px 16px;font-family:'Nunito',sans-serif;font-size:.95rem;transition:border-color .2s;outline:none;}
        .input-group input:focus{border-color:var(--primary-green);}
        .form-actions{grid-column:1 / -1;display:flex;gap:12px;margin-top:10px;}
        .save-btn{background:var(--primary-green);color:#ffffff;border:none;border-radius:999px;padding:12px 28px;font-family:'Nunito',sans-serif;font-weight:800;font-size:.95rem;cursor:pointer;transition:all .2s;}
        .save-btn:hover{background:var(--primary-green-border);}
      ...
      <nav>
        ...
        <div className="nav-links">
          ...
          <Link href="/doctor/profile" className="active">PROFILE</Link>
            ...
            {isEditing ? (
              <div className="edit-form">
                ...
                <div className="input-group">
                  ...
                  <input type="text" name="location" value={formData.location} onChange={handleChange} placeholder="e.g. Mumbai" />
                </div>
                <div className="input-group">
                  <label>Qualifications</label>
                  <input type="text" name="qualification" value={formData.qualification} onChange={handleChange} placeholder="e.g. MBBS, MD" />
                </div>
              ...
              <div>
                ...
                <div className="info-grid">
                  ...
                  <div className="info-row">
                    <div className="icon">⚕️</div>
                    <span><strong>Specialty:</strong> {user.specialization || 'Not specified'}</span>
                  </div>
                  <div className="info-row">
                    <div className="icon">⭐</div>
                    <span><strong>Exp:</strong> {user.experience ? `${user.experience} Years` : 'Not specified'}</span>
                  </div>
                  <div className="info-row">
                    <div className="icon">📍</div>
                  ...
                  <div className="info-row">
                    <div className="icon">🎓</div>
                    <span><strong>Degrees:</strong> {user.qualification || 'Not specified'}</span>
                  </div>
                </div>
